...
__global__ void alignmentOnGPU (
    ...
    int64_t DP_STRIDE = (int64_t)maxSeqLen + 1;
    int64_t tbDirOffset = (int64_t)pair * DP_STRIDE * DP_STRIDE;  // full table
    ...
    // wf buffer zeroed on host before launch
...
void GpuAligner::alignment() {
    ...
    const size_t DP_STRIDE = (size_t)longestLen + 1;

    // Allocate d_tbDir before kernel launch
    ...
    // Allocate d_wf BEFORE kernel launch, zero d_wf after allocation
    ...
    // Zero-initialize so no stale values affect first diagonal
    err = cudaMemset(d_wf_local, 0, wf_bytes);
    if (err != cudaSuccess) {
        fprintf(stderr, "GPU_ERROR (memset d_wf): %s\n", cudaGetErrorString(err));
        exit(1);
    }

    size_t smem_bytes = 2 * longestLen * sizeof(char);
    printf("longestLen=%d DP_STRIDE=%zu dir_bytes=%.1f MB smem_bytes=%zu\n",
           longestLen, DP_STRIDE, dir_bytes / 1048576.0, smem_bytes);

    // 4. Launch kernel — all buffers valid now
    alignmentOnGPU<<<numBlocks, blockSize, smem_bytes>>>(
        d_info, d_seqLen, d_seqs, d_tb, d_tbDir_local, d_wf_local
    );
    ...
    // 5. Transfer traceback from device
    TB_PATH tb_paths = transferTB2Host();
    cudaDeviceSynchronize();

    // 6. Reconstruct aligned strings
    getAlignedSequences(tb_paths);

    // 7. Free temporary GPU buffers
